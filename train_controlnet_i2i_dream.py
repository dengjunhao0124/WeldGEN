...
def log_validation(
    ...
    # pipeline.load_lora_weights(dreambooth_lora)
    # pipeline2.load_lora_weights(dreambooth_lora)
    ...
    image_ori = pipeline2("A PHOTO OF <w*>", num_inference_steps=25, generator=generator).images[0]
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        for _ in range(args.num_validation_images):
            with inference_ctx:
                image = pipeline(
                    validation_prompt, validation_image, num_inference_steps=25, generator=generator
                ).images[0]

            images.append(image)

        images_save.extend(images)
        image_logs.append(
            {"validation_image": validation_image, "images": images, "validation_prompt": validation_prompt}
        )

    tensors = [torch.from_numpy(np.array(img)).permute(2, 0, 1).float() / 255.0 for img in images_save]

    # 合并张量 (自动按网格排列)
    grid_tensor = torchvision.utils.make_grid(tensors, nrow=2, padding=10, pad_value=1.0)

    # 保存合并后的图像
    torchvision.utils.save_image(grid_tensor, f"{args.output_dir}/img_{step}.jpg")
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    def preprocess_train(examples):
        ...
        conditioning_images = [Image.open(os.path.join(args.train_data_dir, "train", random.choice(image) + ".png")).convert("RGB")
